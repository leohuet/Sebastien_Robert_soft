...
void setup(){
  ...
  SerialPort.begin(256000, SERIAL_8N1, RADAR_RX_PIN, RADAR_TX_PIN); //UART for monitoring the radar
  delay(500);
  ...
  moving_max_distance = new PersistentValue("moving max distance", ControlColor::Wetasphalt, 4, 0, 7, radar_tab);
  stationary_max_distance = new PersistentValue("stationary max distance", ControlColor::Wetasphalt, 4, 0, 7, radar_tab);
